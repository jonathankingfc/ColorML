...
class conv2d():
    ...
    def __init__(self, input_channel, output_channel, filter_size, stride):
        ...
        self.weight = np.random.normal(size=(
            self.output_channel, self.input_channel, self.filter_size, self.filter_size))
    ...
    def forward(self, X):
        ...
        self.output_size = int(
            (self.input_size - self.filter_size) / self.stride) + 1
        self.output = np.zeros(
            [self.mini_batch, self.output_channel, self.output_size, self.output_size])
                        ...
                        for j in range(0, self.output_size):
                            ...
                            for idx1, p in enumerate(range(i * self.stride, i + self.filter_size)):
                                for idx2, q in enumerate(range(j * self.stride, j + self.filter_size)):
                                    sum += (self.X[batch][in_channel][p][q] *
                                            self.weight[out_channel][in_channel][idx1][idx2])
                            self.output[batch][out_channel][int(
                                i / self.stride)][int(j / self.stride)] += sum
        return self.output
                        ...
                        for j in range(0, self.filter_size):
                            ...
                            for idx1, p in enumerate(range(i, i + beforeDelta.shape[2])):
                                for idx2, q in enumerate(range(j, j + beforeDelta.shape[3])):
                                    nowDelta += (self.X[batch][in_channel][p][q]
                                                 * beforeDelta[batch][out_channel][idx1][idx2])
                            self.weight[out_channel][in_channel][int(
                                i / self.stride)][int(j / self.stride)] -= (learning_rate * nowDelta)

        rotateDelta = np.rot90(np.rot90(beforeDelta))
        newDelta = np.zeros(
            [self.mini_batch, self.input_channel, self.input_size, self.input_size])
...
class max_pooling():
    ...
    def forward(self, X):
        ...
        self.upsampling = np.zeros(
            [self.mini_batch, self.pooling_input_channel, self.pooling_input_size, self.pooling_input_size])

        pooling_output_size = int(self.pooling_input_size / self.filter_size)
        result = np.zeros([self.mini_batch, self.pooling_input_channel,
            ...
            for k in range(self.pooling_input_channel):
                for i in range(0, self.pooling_input_size - 1, self.filter_size):
                    for j in range(0, self.pooling_input_size - 1, self.filter_size):
                        ...
                        result[batch][k][int(
                            i / self.filter_size)][int(j / self.filter_size)] = max
        return result
                        ...
                        for p in (range(i, i + self.filter_size)):
                            for q in (range(j, j + self.filter_size)):
                                if self.upsampling[batch][k][p][q] != 0:
                                    self.upsampling[batch][k][p][q] *= (
                                        delta[batch][k][int(i / self.filter_size)][int(j / self.filter_size)])
...
def softmax(z):
    ...
    s = np.max(z, axis=1)
    # necessary step to do broadcasting #각 row 마다 max값 구함.
    s = s[:, np.newaxis]
...
class cross_entropy():
    ...
    def forward(self):
        ...
        for i in range(10):
            print('True ', np.argmax(self.y[i]), " predict ", np.argmax(
                self.y_predict[i]), np.argmax(self.y[i]) == np.argmax(self.y_predict[i]))
